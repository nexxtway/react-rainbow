...
describe('<Radio />', () => {
    ...
    it('should set the isChecked passed to the checked prop of the input element', () => {
        const component = mount(<Radio isChecked />);
    ...
    it('should have the right classNames when in the span element of the label when is disabled', () => {
        const component = mount(<Radio label="radio label" disabled />);
        expect(
            component
                .find(
                    'span[className="rainbow-radio-group_radio-label rainbow-radio-group_radio-label--disabled"]',
                )
                .exists(),
        ).toBe(true);
    });
