...
describe('SelectableHeader', () => {
    ...
    it('should set checked to true in PrimitiveCheckbox component when bulkSelection is true', () => {
        const component = mount(<SelectableHeader bulkSelection />);
        expect(component.find('PrimitiveCheckbox').prop('checked')).toBe(true);
    });
    it('should set checked to false in PrimitiveCheckbox component when bulkSelection is false', () => {
        const component = mount(<SelectableHeader bulkSelection={false} />);
        expect(component.find('PrimitiveCheckbox').prop('checked')).toBe(false);
    });
    it('should set checked to "indeterminate" in PrimitiveCheckbox component when bulkSelection is "indeterminate"', () => {
        const component = mount(<SelectableHeader bulkSelection="indeterminate" />);
    ...
    it('should call the right event when bulkSelection is false', () => {
    ...
    it('should call the right event when bulkSelection is other than false', () => {
        const values = ['indeterminate', true, 'abc'];
        values.forEach(value => {
            const onSelectAllRowsMockFn = jest.fn();
            const onDeselectAllRowsMockFn = jest.fn();
            const component = mount(
