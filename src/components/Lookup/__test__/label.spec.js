...
describe('<LookupLabel />', () => {
    ...
    it('should add the right class names when readOnly is passed', () => {
        ...
        expect(component.find('.rainbow-lookup_input-label.rainbow-lookup_input-label-read-only').exists()).toBe(true);
    });
    it('should add the right class names when hideLabel is passed', () => {
        const component = mount(
            <Label label="Input Label" hideLabel />,
        );
        expect(component.find('.rainbow-lookup_input-label.rainbow-lookup_input-label--hide').exists()).toBe(true);
    });
});
