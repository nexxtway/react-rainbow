...
describe('normalizeMinutes', () => {
    ...
    it('should prefix with 0 when the value is less than 10', () => {
        ...
        values.forEach((value, index) => expect(normalizeMinutes(value, '35')).toBe(expects[index]));
    });
    it('should return the same value when the value passed is between 10 and 59', () => {
        ...
        values.forEach((value, index) => expect(normalizeMinutes(value, '02')).toBe(expects[index]));
    });
});
